-- =====================================================
-- Migration: ULTIMATE MASTER RECONSTRUCTION (V8)
-- Resolve: RPC 404s, BIGINT vs UUID Conflicts, Data Loading
-- =====================================================
...
-- 1. UTILITY: Drop ALL Secure RPCs to avoid signature mismatch
DO $$ DECLARE
    r record;
BEGIN
    FOR r IN (SELECT proname, oid FROM pg_proc WHERE pronamespace = 'public'::regnamespace 
              AND proname IN (
                  ...
                  'sync_students_to_group_secure', 'get_users_secure', 'admin_update_user'
              ))
    LOOP
...
DECLARE
    v_role TEXT; v_full_name TEXT; v_total_users BIGINT; v_total_assistants BIGINT; v_total_students BIGINT;
    v_total_feedback BIGINT; v_pending_approval BIGINT; v_my_classes BIGINT; v_my_students BIGINT; v_attendance_rate DECIMAL;
BEGIN
    ...
    END IF;
    RETURN jsonb_build_object('user_role', v_role, 'user_name', v_full_name, 'total_users', v_total_users, 'total_assistants', v_total_assistants, 'total_students', v_total_students, 'total_feedback', v_total_feedback, 'pending_approval', v_pending_approval, 'total_kelas', v_my_classes, 'total_students_under_me', v_my_students, 'attendance_rate', v_attendance_rate);
END; $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.users WHERE id = p_viewer_id AND role IN ('koordinator', 'asisten', 'sekretaris', 'k3')) THEN RAISE EXCEPTION 'Access Denied'; END IF;
    RETURN QUERY SELECT inv.id::BIGINT, inv.name::TEXT, inv.condition::TEXT, inv.quantity::INTEGER, inv.location::TEXT, inv.updated_at::TIMESTAMP WITH TIME ZONE FROM public.inventory_items inv ORDER BY inv.name ASC;
END; $$;
...
BEGIN
    ...
    RETURN QUERY SELECT fr.id::BIGINT, fr.title::TEXT, fr.amount::NUMERIC, fr.type::TEXT, fr.category::TEXT, COALESCE(fr.date, fr.created_at)::TIMESTAMP WITH TIME ZONE FROM public.financial_records fr ORDER BY fr.created_at DESC;
END; $$;
...
BEGIN
    ...
    IF p_id = 0 THEN
        INSERT INTO public.financial_records (title, amount, type, category, date, created_at) VALUES (p_title, p_amount, p_type, p_category, now(), now());
    ELSE
        UPDATE public.financial_records SET title = p_title, amount = p_amount, type = p_type, category = p_category WHERE id = p_id;
    END IF;
END; $$;

-- 5. CLASS MANAGEMENT (RESTORED)
CREATE OR REPLACE FUNCTION public.sync_students_to_group_secure(p_caller_id BIGINT, p_class_code TEXT, p_major TEXT, p_schedule_id TEXT)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.users WHERE id = p_caller_id AND role IN ('koordinator', 'asisten')) THEN RAISE EXCEPTION 'Access Denied'; END IF;
    -- Logic to sync students based on criteria
    INSERT INTO public.group_members (student_id, schedule_id, created_at)
    SELECT u.id, p_schedule_id::UUID, now() FROM public.users u 
    WHERE u.role = 'praktikan' AND u.class_code = p_class_code AND u.division = p_major
...
BEGIN
    RETURN QUERY SELECT u.id::BIGINT, u.full_name::TEXT FROM public.users u WHERE u.role IN ('asisten', 'koordinator')
    AND EXISTS (SELECT 1 FROM public.assistant_availability aa WHERE aa.user_id = u.id AND aa.day_of_week = p_day AND aa.is_available = true)
    AND NOT EXISTS (SELECT 1 FROM public.group_assistants ga JOIN public.schedules s ON ga.schedule_id = s.id WHERE ga.assistant_id = u.id AND s.day_of_week = p_day AND (s.start_time, s.end_time) OVERLAPS (p_start::TIME, p_end::TIME));
END; $$;

-- 6. SETTINGS & ATTENDANCE (Fixed Cast)
CREATE OR REPLACE FUNCTION public.get_system_settings_full_secure(p_viewer_id BIGINT)
RETURNS TABLE (semester_active TEXT, announcement TEXT, is_recruitment_open BOOLEAN) LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.users WHERE id = p_viewer_id AND role IN ('koordinator', 'sekretaris')) THEN RAISE EXCEPTION 'Access Denied'; END IF;
...
BEGIN
    ...
    UPDATE public.system_settings SET semester_active = p_semester_active, announcement = p_announcement, is_recruitment_open = p_is_recruitment_open, updated_at = now() WHERE id = (SELECT id FROM public.system_settings LIMIT 1);
END; $$;
